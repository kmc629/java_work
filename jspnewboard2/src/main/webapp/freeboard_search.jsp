...
<body>
	...
	<form method=post name=search action="freeboard_search.jsp">
		<table border=0 width=95%>
			<tr>
				<td align=center width=50% valign=bottom>[<A
					href="freeboard_list.jsp">자유 게시판(일반모드)</A>]
				</td>
				<td align=left width=50% valign=bottom><font size=2 face=굴림>
						<select name=stype>
							<%
							String cond = null;
							int what = 1;
							...
							if (request.getParameter("stype") != null) {
								...
								if (what == 7) {
									out.println("<OPTION value=7 selected>이룸/제목/내용");
									cond = "where name like '%" + val + "%'";
									cond = cond + " or subject like '%" + val + "%'";
									cond = cond + " or content like '%" + val + "%'";
								} else
